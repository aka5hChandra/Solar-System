...
vec4 light= vec4(rim+ambientMaterial*ambientComponent+diffuse +specular,1) ;
  ...
  //Render lighting based flag "light check"
  if(lightCheck)
  {
	  //Night lights for planet Earth
if(nightLights)
...
color= vec4(light)*texture2D(texture,frag_in.geoGenratedTexCords.xy );
}
else
color=texture2D(texture,frag_in.geoGenratedTexCords.xy );
